...
/**
 * 构造提醒脚本片段（2026-08-25 修因：interval 符号）。
 * alert_minutes 语义 =「提前 N 分钟提醒」→ AppleScript trigger interval 必须为负
 * （正值 = 事件开始后触发，等于无效提醒）。未传 alertMin 返回注释占位。
 */
export function buildAlertScript(alertMin) {
  if (alertMin === undefined || alertMin === null || alertMin === '') return '-- no alert'
  const mins = Math.abs(Number(alertMin))
  if (!Number.isFinite(mins) || mins <= 0) return '-- no alert'
  return `make new display alarm at end of display alarms of newEvent with properties {trigger interval:-${mins}}`
}
...
async function addEvent(args) {
  ...
  const alertScript = buildAlertScript(alertMin)
